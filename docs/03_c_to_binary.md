...
## 5. ASIC / CPU 설계 관점에서 왜 중요?

- 우리가 설계하는 **CPU, NPU 코어가 실제로 실행하는 건**  
  → 위 툴체인이 뱉어낸 **RISC-V 기계어(ELF의 .text 섹션)**.
- `objdump -S`로:
  - C → RISC-V 명령어 매핑을 눈으로 확인
  - “이 ISA, 이 파이프라인이면 이런 패턴의 코드가 많이 나오겠구나” 감 잡기
- 결국:
  - ISA 설계 / 파이프라인 설계 / 브랜치, 로드-스토어 성능을 볼 때  
    **컴파일러가 실제로 어떤 코드 패턴을 생성하는지** 이해하는 게 핵심.

이 정도만 깃허브에 정리해 두면  
“툴체인 큰 흐름 알고 있고, objdump로 코드 패턴 보는 습관 있다” 정도는 어필할 수 있음.
